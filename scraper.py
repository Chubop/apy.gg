...
class Crawler:
    ...
    async def scroll(self, direction):
        # change made: removed sensitive casing for manual user input cases
    ...
    async def click(self, id):
        ...
        element = await self.page_element_buffer.get(int(id))
        if element:
    ...
    async def crawl(self):
        ...
        # let's filter further to remove anything that does not hold any text nor has click handlers + merge text from
        # leaf#text nodes with the parent
        ...
        for element in elements_in_view_port:
            ...
            # made change here
            if (
                    converted_node_name not in ["button", "link", "input", "img", "textarea"]
                    and meta == ""
...
def extract_child(input_string, start_symbol='>', end_symbol='<'):
