...
async def citizens_signup(info):
    def app_loading_hidden(page):
        return page.evaluate("""() => {
            const appLoadingElement = document.querySelector('#appLoading');
            return appLoadingElement && appLoadingElement.style.display === 'none';
        }""")

    bank = SignUp("Citizens",
                  "https://application.citizensaccess.com/openaccount/?productType=3000&productTerm=&openingAmount#!/acctopening")
    user = User(info)
    ...
    async with async_playwright() as p:
        ...
        time.sleep(5)
        # Wait for the div element with class "dimBackground" to have display: none
        await page.is_hidden('#appLoading')

        await page.fill("#amount0", "0.01")

        if await page.is_visible('#appLoading'):
            await page.is_hidden('#appLoading')
            await page.fill("#amount0", "0.01")
        ...
        await page.fill('#lastName', user.last_name)
        await page.fill('#zipCode2', user.contact_info.zip)
        await page.fill('#city', user.contact_info.city)
        await page.fill('#primary2', user.contact_info.address)

        if user.different_mailing_address:
            await page.click("label[for='isMailingAddress']")

            await page.fill('#mailingZipCode2', user.mailing_info.zip)
            await page.fill('#mailing2', user.mailing_info.address)
            await page.fill('#phone', user.contact_info.phone_number)
            await page.fill("input[name='mailingCity']", user.mailing_info.city)
            await page.select_option('#isThisMobile', 'Mobile')
            await page.fill('#email', user.contact_info.email)
            # await page.select_option(convert_tag(user.mailing_info.state))

        await page.fill('#dob', user.date_of_birth.replace('-', '/'))
        # TODO: more occupation support
        await page.select_option('#occupation', user.employment_info.occupation)
        await page.evaluate("document.getElementById('caxssn').value = '{}'".format(user.ssn))
        await page.type('#caxssn', user.ssn)
        await page.click("button[cbdata-reason='personal-info-continue']")

        # Next page
        await page.wait_for_selector("button:text('Continue')")
        await page.click("button:text('Continue')")

        await page.wait_for_selector("label[for='electronicCheck']")
        await page.click("label[for='electronicCheck']")

        await page.wait_for_selector("label[for='termsCheck']")
        await page.click("label[for='termsCheck']")

        await page.wait_for_selector("label[for='usResident']")
        await page.click("label[for='usResident']")

        await page.wait_for_selector("select[name='sourceOfIncomeOpts']")
        await page.select_option("select[name='sourceOfIncomeOpts']", user.employment_info.source_of_income)

        await page.select_option("select[name='multiAcctUsage']", "Long-Term Savings (Education, Home, etc.)")

        await page.select_option("select[name='isIntlTransSelected']", "No")
